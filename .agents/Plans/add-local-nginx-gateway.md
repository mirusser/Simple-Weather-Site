...
- ✅ Static/config checks:
  - ✅ `docker compose --project-directory src -f src/deploy/docker-compose.local.yml config` — valid
  - ✅ rendered config contains `gateway`, `8080:80`, `nginx.local.conf` mount, and `ASPNETCORE_FORWARDEDHEADERS_ENABLED` on both services.
