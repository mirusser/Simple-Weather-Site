...
ECR_PREFIX="${ECR_PREFIX:-sws}" # namespace/prefix in ECR, e.g. sws/oauthserver
...
ecr_login() {
    if [ "$PUSH_TO_ECR" = "1" ]; then
        ...
        echo -e "${BLUE}Logging into ECR registry: $ECR_REGISTRY${NC}"
...
ensure_ecr_repo() {
    local repo="$1" # e.g. sws/oauthserver
...
tag_and_push_image() {
    if [ "$PUSH_TO_ECR" = "1" ]; then 
        # Args:
        #  $1 local image name (e.g. oauthserver)
        #  $2 repo name (e.g. oauthserver) -> will become $ECR_PREFIX/oauthserver
        local local_image="$1"
        local repo_name="$2"
...
IMAGE_TAG=$IMAGE_TAG
EOF
    echo -e "${GREEN}Wrote $BASE_DIR/.env.prod (copy this to EC2 with your prod compose)${NC}"
fi
}
...
# If PUSH_TO_ECR is enabled: tag and push to AWS ECR
